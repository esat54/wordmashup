...
export default function AllWords() {
  ...
  useEffect(() => {
    loadData();
  }, [limit, type, favoriteFilter, debouncedSearchTerm]);

  const loadData = async () => {
    try {
      setLoading(true);
      const response = await wordsApi.getWords({ limit, type, favoriteFilter, searchTerm: debouncedSearchTerm, }) as any;
            ...
            <div key={word._id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <Star
                      className={`w-5 h-5 ${word.favorite ? "fill-current text-yellow-500" : "text-gray-400"}`}
                    />
                  </button>
